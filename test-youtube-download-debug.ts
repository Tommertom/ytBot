 ...
 */
function executeYtDlp(args: string[]): Promise<{ stdout: string; stderr: string; code: number }> {
...
async function testGetVideoInfo() {
    ...
    try {
        ...
        if (result.code === 0) {
            ...
            console.log(`  Duration: ${info.duration} seconds (${(info.duration / 60).toFixed(2)} minutes)`);
            ...
            if (info.filesize) {
                ...
                console.log(`  Approx file size: ${(info.filesize_approx / 1024 / 1024).toFixed(2)} MB`);
            }
...
async function testDownloadVerbose() {
    console.log('\n=== TEST 3: Download with verbose output ===');
    const outputTemplate = path.join(OUTPUT_DIR, 'test-verbose-%(title)s.%(ext)s');
 ...
 */
async function testDownloadNoSizeLimit() {
    console.log('\n=== TEST 5: Download without size limit (to see actual behavior) ===');
    const outputTemplate = path.join(OUTPUT_DIR, 'test-nosize-%(title)s.%(ext)s');
    ...
    try {
        ...
        if (result.code === 0) {
            ...
            for (const line of lines) {
                const trimmedLine = line.trim();
                if (trimmedLine && fs.existsSync(trimmedLine) && trimmedLine.endsWith('.mp3')) {
                    const stats = fs.statSync(trimmedLine);
                    const sizeMB = (stats.size / 1024 / 1024).toFixed(2);
                    console.log(`✓ Downloaded file: ${trimmedLine}`);
                    console.log(`✓ Actual file size: ${sizeMB} MB`);

                    if (stats.size / 1024 / 1024 > MAX_FILE_SIZE_MB) {
                        console.log(`⚠️  File exceeds ${MAX_FILE_SIZE_MB} MB limit`);
                    }
...
async function runTests() {
    ...
    const results = {
        ...
        listDir: await listOutputDirectory(),
        noSizeLimit: await testDownloadNoSizeLimit(),
        finalList: await listOutputDirectory()
    };
    ...
    console.log(`Download without size limit: ${results.noSizeLimit ? '✓' : '✗'}`);
